                        ...
                        <form id="form">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="md-form form-group">
                                        <input type="hidden" class="form-control" id="id_agenda" name="id_agenda" value="<?= $id; ?>">
                            ...
                            <div class="form-row">
                                ...
                                <div class="col-md-6">
                                    <!-- Material input -->
                                    <div class="md-form form-group">
                                        <select class="mdb-select md-form" id="atasan" name="atasan" searchable="Search here..">
                                            <option id="opsi" value="" selected disabled>Pilih Atasan Langsung</option>
                                            <?php

                                            $jns_jabatan = $this->session->userdata('jns_jabatan');
                                            $unit =  $this->session->userdata('unit');
                                            $query1 = $this->db->query("SELECT nama, nip, jabatan, jns_jabatan
                                            FROM t_pegawai WHERE unit = '$unit' AND (jns_jabatan='PENGAWAS' OR jns_jabatan='JTP PRATAMA' OR jns_jabatan='ADMINISTRATOR')");

                                            $query2 = $this->db->query("SELECT nama, nip, jabatan, jns_jabatan
                                            FROM t_pegawai WHERE unit= '$unit' AND ( jns_jabatan='JTP PRATAMA' OR jns_jabatan='ADMINISTRATOR')");

                                            $query3 = $this->db->query("SELECT nama, nip,jabatan,jns_jabatan
                                            FROM t_pegawai WHERE unit= '$unit' AND jns_jabatan='JTP PRATAMA'");

                                            if ($jns_jabatan == "PELAKSANA" || $jns_jabatan == "FUNGSIONAL") {
                                                foreach ($query1->result() as $list) { ?>
                                                    <option value="<?= $list->nip; ?>"><?= $list->nama . " - " . $list->jabatan; ?> </option>;
                                                <?php }
                                            }
                                            if ($jns_jabatan == "PENGAWAS") {
                                                foreach ($query2->result() as $list) { ?>
                                                    <option value="<?= $list->nip; ?>"><?= $list->nama . " - " . $list->jabatan; ?> </option>;
                                                <?php }
                                            }
                                            if ($jns_jabatan == "ADMINISTRATOR") {
                                        ...
                                        <label class="mdb-main-label">Atasan Langsung</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <!-- Material input -->
...
<script type="text/javascript">
    ...
    function simpan1() {
        ...
        if ($('#atasan :selected').val() == "") {
            swal('Gagal', 'Atasan Masih Kosong', "error");
            return false;
        }
        if ($('#tgl').val() == "") {
